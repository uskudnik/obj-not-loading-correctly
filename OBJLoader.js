...
THREE.OBJLoader.prototype = {
	...
	load: function ( url, onLoad, onProgress, onError ) {
		...
		loader.load( url, function ( text ) {
			...
			var pp = scope.parse( text );
			pp.traverse(function(child) {
				// console.log('pp traverse', child.name);
			});
			// console.log('parseme', );
	...
	parse: function ( data ) {
		...
		for ( var i = 0; i < lines.length; i ++ ) {
			...
			if ( line.length === 0 || line.charAt( 0 ) === '#' ) {
				...
				console.log('obj name', line.substring( 2 ).trim());
		...
		group.traverse(function(child){
			// console.log('grup traverse', child);
		})
